...
public:
  Class_ get_cls() { return cls; }

  void set_cls(Class_ cls) { this->cls = cls; }

  int get_cls_attrs_size() { return cls_attrs.size(); }

  int get_mth_args_size() { return mth_args.size(); }

  void add_cls_attr(attr_class *attr) { cls_attrs.push_back(attr); }

  void add_mth_arg(Formal formal) { mth_args.push_back(formal); }

  void clear_mth_args() { mth_args.clear(); }

  void push_stack_symbol(Symbol name) { stack_symbols.push_back(name); }

  void pop_stack_symbol() { stack_symbols.pop_back(); }
  ...
  // returns argument's position on the vector (starting from 0) or -1 if not
  // found
  ...
  // returns attribute's position on the vector (starting from 0) or -1 if not
  // found
